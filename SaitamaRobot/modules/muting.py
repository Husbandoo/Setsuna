...
from Setsuna import LOGGER, TIGERS, dispatcher
...
__help__ = """
*Admemes only:*
 • `/mute <userhandle>`*:*tapes a user's whole face. Can also be used as a reply, muting the replied to user.
 • `/tmute <userhandle> x(m/h/d)`*:* tapes a user for x time. (via handle, or reply). `m` = `minutes`, `h` = `hours`, `d` = `days`.
